...
for k in range(nr_tests):
    ...
    while piece_of_water < pieces_of_water:
        if piece_of_water + 1 == pieces_of_water:
            ...
            # if coordinates too close to existing water surface, generate new coordinates
            for p in range(len(water)):
                if water[p].x_min > x_min:
                    while water[p].x_min > x_min + 50:
                        x_min = random.randrange(10, 2 * (0.8 * bound_x - 20)) * 0.5
                        print "xminp" , water[p].x_min
                        print "newxmin", x_min
            y_min = random.randrange(30, 2 * (0.8 * bound_y - 20)) * 0.5
            for p in range(len(water)):
                if water[p].y_min > y_min:
                    while water[p].y_min > y_min + 50:
                        y_min = random.randrange(10, 2 * (0.8 * bound_y - 20)) * 0.5
                        print "xminy", water[p].y_min
                        print "newymin", y_min
